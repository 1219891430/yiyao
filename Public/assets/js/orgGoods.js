...
function queryGoodsList(url, brand, goods, depot_id, cust_id, staff_id)
{
	$.ajax({
		url:url,
		type:"post",
		dataType:"json",
		data:{brand:brand, goods:goods, depot_id:depot_id, cust_id:cust_id, staff_id:staff_id},
...
//			default_goods_code = convert[j]["goods_code"];
	...
	if(is_show_stock == 1){
		goodsHTML += "<td><input class='w50 tr num' type='text' value='"+default_goods_stock+"'></td>";
	}
	goodsHTML += "</tr>";
	return goodsHTML;
}
...
function addGoodsIntoOrderGoodsList(depotID, checkStockURL, is_show_stock, is_show_last_price)
{
	...
	for(var i=0;i<checkbox.length;i++)
    {
		...
		var cv_data=checkbox.eq(i).parent().siblings(".cv_data").val();
		if(cv_data=="undefined"){
			alert("未设置"+goods_name+"价格");
			return;
		}
