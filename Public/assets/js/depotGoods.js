...
function addGoodsToTableBtnInit(depotID, checkStockURL, is_show_stock, is_show_last_price)
{
    ...
    $("#goods-add").click(function(){ 
    	...
    	addGoodsIntoOrderGoodsList(depotID, checkStockURL, is_show_stock, is_show_last_price);
    })
}
	...
	$.ajax({
		...
		success:function(data){
			...
			$.each(data["data"],function(i){	
				...
				var goods_area = data['data'][i]['goods_area'];
				...
				conHTML += "<input class='goods_area' type='hidden' value='"+goods_area+"'>";
...
function createGoodsHTML(goods_id, goods_code,goods_name, goods_spec, goods_area,convert, is_show_stock, is_show_last_price)
{
	// 商品默认单位信息
	var optionHTML = '';
	var default_cv_id = 0;
...
function addGoodsIntoOrderGoodsList(depotID, checkStockURL, is_show_stock, is_show_last_price)
{
	...
	for(var i=0;i<checkbox.length;i++)
    {
		...
		var goods_area=checkbox.eq(i).parent().siblings(".goods_area").val();
		
		// 拼接商品HTML
		goodsHTML += createGoodsHTML(goods_id, goods_code,goods_name, goods_spec, goods_area,convert, is_show_stock, is_show_last_price);
    }
