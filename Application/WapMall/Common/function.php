...
// 最新添加
function getNewestGoods($depot_id, $limit=1000,$class_id=0,$brand_id=0) {
	if($brand_id){
		$where['gi.brand_id'] = $brand_id;
	}
	if($class_id){
...
// 热销品类
function getClass($depot_id, $limit=1000,$parent_class=0) {
    $where['dc.repertory_id'] = $depot_id;
    $where['gc.parent_class'] = $parent_class;
    $where["gc.is_close"] = 0;
    //$where['po.order_status'] = 3;

    $classes = M('goods_class')->alias('gc')
        // 出库
        ->join('__DEPOT_CLASS__ as dc on dc.class_id=gc.class_id')
        ->field('gc.*')
        ->where($where)
        ->limit($limit)
        ->select();
...
function getHotBrandByClass($depot_id, $class_id, $limit=1000) {
    $where['po.repertory_id'] = $depot_id;
    $where["gb.is_close"] = 0;
...
// 检查字符串是否在数组的某个键值中
function strInArray($str, $key, $array) {
    $status = false;
    if (is_array($array)) {
