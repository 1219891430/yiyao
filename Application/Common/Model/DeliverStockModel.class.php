...
/*******************************************************************
 ** 文件名称: DeliverStockModel.class.php
 ** 功能描述: 平台车销配送实时车库操作公共模型类
 ** 创建人员: richie
 ** 创建日期: 2016-08-03
*******************************************************************/
...
class DeliverStockModel extends Model {

	// 数据主表:终端退货预单数据表
	protected $tableName = 'car_stock';
	
	/**
	 * 更新配送车存
	 *
	 * @param int 		$staff_id  		配送人员ID
	 * @param array  	$goods_list[] = array('cv_id'=>'', 'goods_id'=>'', 'goods_num'=>'')
	 * @param string 	$stock_type		1车存申请(+), 2终端退货(+), 3调换退货(+), 4车销(-), 5调换出货(-), 6车销退库(-)
	 * @return bool		返回成功失败
	 */
	public function updateCarInfo($staff_id, $goods_list, $stock_type)
	{
		// 循环商品进行更新
		foreach($goods_list as $item)
		{
			...
			// 是否已经存在
			$stockInfo = M('car_stock')->where("staff_id = $staff_id AND goods_id = $goods_id")->find();
			...
			// 更新数量
			if($stock_type <=3 ){ $num = $stockNumber + $changeGoodsNumber; }
			...
			M('car_stock')->where("staff_id = $staff_id AND goods_id = $goods_id")->save($data);	
