...
/*******************************************************************
 ** 文件名称: CarSalesCheckController.class.php
 ** 功能描述: 经销商PC端车销对账控制器
 ** 创建人员: richie
 ** 创建日期: 2016-07-29
*******************************************************************/
...
class CarSalesCheckController extends Controller {
	...
	{
		$where["role_id"]=3;//角色是业务员
		$where["org_parent_id"]=session("org_parent_id");
		$staffList=M("org_staff")
		     ->field("staff_id,staff_name")
		     ->where($where)->select();
		foreach($staffList as $k=>$vo){
			$res=M('carsales_duizhang')->field("cd_id,start_time,end_time")->where("staff_id=".$vo["staff_id"])->order("cd_id desc")->find();
			
			if($res){
				$staffList[$k]["lastCheck"]=date("Y-m-d H:i:s",$res["end_time"]);
	...
	private function duiZhangHuiZongNew($staff_id,$start_date,$end_date){
		...
		$goodData = M("goods_brand")
			...
			zdb_goods_info.goods_name,zdb_goods_info.goods_spec,zdb_org_goods_convert.cv_id,zdb_org_goods_convert.goods_unit,
			zdb_goods_info.goods_convert_s,zdb_goods_info.goods_convert_m,zdb_goods_info.goods_convert_b,
			zdb_org_goods_convert.goods_unit_type
			")
			->join("zdb_goods_info on zdb_goods_info.brand_id=zdb_goods_brand.brand_id")
			->join("zdb_org_goods_convert on zdb_org_goods_convert.goods_id=zdb_goods_info.goods_id")
		 ...
		 */
		$SCDZList=M()->query("
        select goods_id,cxzh.cd_id,carport,carport_int from zdb_carsales_duizhang_goods as cxzh
		where EXISTS (
		select max(zdb_carsales_duizhang_goods.cd_id) from zdb_carsales_duizhang_goods 
		inner join zdb_carsales_duizhang  on  zdb_carsales_duizhang.cd_id=zdb_carsales_duizhang_goods.cd_id where staff_id=$staff_id 
		and cxzh.cd_id=zdb_carsales_duizhang_goods.cd_id
		group by goods_id 
		)");
		 ...
		 * 今日出库, 按照收货时间
		 */
		$whereB["zdb_carsale_apply.staff_id"]=$staff_id;
		$whereB["apply_status"]=3;
		$whereB["zdb_carsale_apply.accept_time"]=array(array("gt",$start_date),array("lt",$end_date));
		$chukuList=M("carsale_apply")
			->field("zdb_carsale_apply_goods.goods_id,cv_id,goods_unit,sum(apply_num) as number")
			->join("zdb_carsale_apply_goods on zdb_carsale_apply_goods.apply_id=zdb_carsale_apply.apply_id")
			->where($whereB)
			->group("cv_id")
			->order("cv_id desc")
			->select();
		...
		$whereTK["return_status"]=3;
		$whereTK["zdb_carsale_return_stock.check_time"] = array(
			array("gt", $start_date),
			array("lt", $end_date)
		);
		$tuikulist = M("carsale_return_stock")
			->field("goods_unit,sum(goods_num) as number,cv_id,zdb_carsale_return_stock_goods.goods_id")
			->join("zdb_carsale_return_stock_goods on zdb_carsale_return_stock_goods.return_id=zdb_carsale_return_stock.return_id")
			->where($whereTK)
			->group("cv_id")
			->order("cv_id desc")
			->select();
		...
		$whereC['zdb_carsale_orders.create_time']=array(array("gt",$start_date),array("lt",$end_date));		
		$xiaoshouList=M("carsale_orders")
			->field("goods_id,cv_id,unit_name as goods_unit,sum(number) as number")
			->join("zdb_carsale_orders_goods on zdb_carsale_orders_goods.order_id=zdb_carsale_orders.order_id")
			->where($whereC)
			->group("cv_id")
			->order("cv_id desc")
			->select();
		...
		$TCHList=M("carsales_change")
			->field("goods_id,cv_id,sum(number) as number")
			->join("zdb_carsales_change_goods on zdb_carsales_change_goods.change_id=zdb_carsales_change.change_id")
			->where($whereTHH)->group("zdb_carsales_change_goods.cv_id")
			->select();
		...
		//退货数量
		$whereG["zdb_carsales_return.staff_id"]=$staff_id;
		$whereG["zdb_carsales_return.create_time"]=array(array("gt",$start_date),array("lt",$end_date));
		$tuihuoList=M("carsales_return")
			->field("goods_id,cv_id,goods_unit,sum(goods_num) as number")
			->join("zdb_carsales_return_goods on zdb_carsales_return_goods.return_id=zdb_carsales_return.return_id")
			->where($whereG)
			->group("cv_id")
			->order("cv_id desc")
			->select();
		...
		$wheredj["zdb_carsale_orders.create_time"]=array(array("gt",$start_date),array("lt",$end_date));
		$danjiaList=M("carsale_orders_goods")
...
//  	if($cv['small_is_close']==0&&$cv['mid_is_close']==0&&$cv['big_is_close']==0){
	...
	private function sheQianCount($staff_id, $start_date, $end_date)
	{
		...
		$where['is_full_pay']=0;
		$list=M("carsale_orders")->field("cust_id,cust_name,cust_contact,sum(order_total_money) as total_money,sum(order_real_money) as real_money")->where($where)->group("cust_id")->select();
		
		foreach($list as $k=>$v){
			
			$res=M("carsale_orders_qiankuan")->field("sum(price) as qingqianmoney")->where("cust_id=".$v["cust_id"])->find();
	...
	public function duizhangAction(){
		...
		$resTou=M('carsales_duizhang')->field("end_time")->where($whereTou)->order("cd_id desc")->find();
		if($resTou){
...
//			}
		...
		$this->assign("staff_id",$staff_id);
		// 表头
		$wherebt["staff_id"]=$staff_id;
		$staff=M("org_staff")->field("staff_name")->where($wherebt)->find();
		...
		$resSum=M("carsale_orders")->field("sum(order_total_money) as totalMoney,sum(order_real_money) as realMoney")->where($whereOrder)->find();
		$this->assign("resSum",$resSum);
		//赊欠金额
		$whereQian["zdb_carsale_orders.staff_id"]=$staff_id;
		$whereQian["zdb_carsale_orders.create_time"]=array(array("gt",$date1),array("lt",$date2));
		$resQian=M("carsale_orders_qiankuan")->field("sum(price) as total_price")->join("zdb_carsale_orders on zdb_carsale_orders.order_id=zdb_carsale_orders_qiankuan.orderid")->where($whereQian)->find();
		...
		$resTH=M("carsales_return")->field("sum(total_money) as tui_total_money")->where($whereTH)->find();
		...
		$resQK=M("carsale_orders_qiankuan")->field("sum(price) as total_staff_qkmoney")->where($whereQK)->find();
		$total_staff_qkmoney=$resQK["total_staff_qkmoney"];
		$whereQK["qk_type"]=1;//业务员清欠
		$resQK=M("carsale_orders_qiankuan")->field("sum(price) as total_admin_qkmoney")->where($whereQK)->find();
		...
		$listQKCX=M("carsale_orders")->field("order_id,order_total_money,order_real_money")->where($whereQKCX)->select();
		...
		$resQkO=M("carsale_orders_qiankuan")->field("sum(price) as qingqianmoney")->where($whereQKCXId)->find();
		...
		//调换货差价
		$whereChange["zdb_carsales_change.staff_id"]=$staff_id;
		$whereChange["create_time"]=array(array("gt",$date1),array("lt",$date2));
		$whereChange["is_change_in"]=0;
		$ChangeOutRes=M("carsales_change")->field("sum(singleprice*number) as total_in_money")->join("zdb_carsales_change_goods on zdb_carsales_change_goods.change_id=zdb_carsales_change.change_id")->where($whereChange)->find();
		$whereChange["is_change_in"]=1;
		$ChangeInRes=M("carsales_change")->field("sum(singleprice*number) as total_out_money")->join("zdb_carsales_change_goods on zdb_carsales_change_goods.change_id=zdb_carsales_change.change_id")->where($whereChange)->find();
		
		$changeMoney=$ChangeOutRes["total_in_money"]-$ChangeInRes["total_out_money"];
		$this->assign("changeMoney",$changeMoney);
		//对账金额
	...
	public function setDuizhangAction(){
		...
		$cd_id=M("carsales_duizhang")->add($data);
...
//			$dataGood[$k]["goods_num"]=$good["number"];
		...
		$res=M("carsales_duizhang_goods")->addAll($dataGood);	
			...
			M("carsales_duizhang_sheqian")->addAll($dataSheqian);
   ...
   public function historyAction(){
		...
		$where['staff_id']=$staff_id;
		$total=M("carsales_duizhang")->where($where)->count();
		$list=M("carsales_duizhang")->field("cd_id as id,end_time")->where($where)->order("end_time desc")->page($p,$pnum)->select();
		$page=get_page_code($total,$pnum,$p, $page_code_len = 5);
		$whereA["staff_id"]=$staff_id;
		$staff=M("org_staff")->field("staff_name")->where($whereA)->find();
		$this->assign("staff_name",$staff["staff_name"]);
    ...
    public function showHistoryDuizhangAction(){
		...
		$this->assign("id",$id);
		$where["cd_id"]=$id;
		$resDuizhang=M("carsales_duizhang")->where($where)->order("cd_id desc")->find();
		 ...
		 * 表头
		 */
		$wherebt["staff_id"]=$resDuizhang['staff_id'];
		$staff=M("org_staff")->field("staff_name")->where($wherebt)->find();
		
		$this->assign("staff_id",$wherebt["staff_id"]);
		$this->assign("staff_name",$staff["staff_name"]);
		...
		$huizongList=M("carsales_duizhang_goods")
		...
		foreach ($huizongList as $kk=>$vv){
			...
			$huizongGoods=M("carsales_duizhang_goods")
			...
			//var_dump($huizongList[$kk]["Goods"]);
		}
		
		$this->assign("huizongList",$huizongList);
		//赊欠
		$sheqianList=M("carsales_duizhang_sheqian")->where($where2)->select();
