<?php if (!defined('THINK_PATH')) exit();?><div class="modal-content modal_850">
    ...
    <form action="" id="submit_form" method="post">
        <div class="modal-body modal_850">
            ...
            <?php if(is_array($resGoods)): $i = 0; $__LIST__ = $resGoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gvo): $mod = ($i % 2 );++$i;?><tr class='tr_operate'>
                    ...
                    <td><input class='w50 tr goods_num' type='text' value='<?php echo (getGoodsNum($gvo["in_num"])); ?>'></td>
	...
	$("#find_goods").click(function(){
		if($("#goods").val()!="")
			queryGoodsList("<?php echo U('Admin/GoodsInfo/selGoods');?>", $("#brand").val(), $("#goods").val(), $("#depot_id").val(), 0, 0, $("#org_id").val());
		else
			alert("请填写商品");
	});
	...
	$("#brand").change(function(){
		if($("#brand").val()==0 && $("#goods").val()==""){
			...
			queryGoodsList("<?php echo U('Admin/GoodsInfo/selGoods');?>", $("#brand").val(), '', $("#depot_id").val(), 0, 0, $("#org_id").val());
		}
	});
	...
	$("#create_form").click(function(){
        ...
        function depotIn(){
            ...
            else
            {
                ...
                var data={depot_in_id:<?php echo ($depot_in_id); ?>,types:$("#in_types").val(),depot_id:$("#depot_id").val(),org_id:$("#org_id").val(),goods_info:aGoodsData,remark:$("#in_remark").val()}
                
                console.log(data);
                ajaxDataAUD("/index.php/Admin/DepotIn/editex",data,true)
            }
        }
