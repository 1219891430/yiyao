...
<?php if(is_array($sub_memu)): $i = 0; $__LIST__ = $sub_memu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rightsubvo): $mod = ($i % 2 );++$i;?><dl>
    ...
    <div class="row-fluid main-content">
        <div class="sel-data mb20">
            ...
            <input type="hidden" id="lineID" name="lineID" value="<?php echo ($line_info["line_id"]); ?>" />
        ...
        <td><?php echo ($line_info["line_name"]); ?></td>
        ...
        <li><a class="collapsed collapse-menu icons-href" href="javascript:void(0)" onClick="delShops('<?php echo ($line_info[line_id]); ?>','<?php echo ($da[cust_id]); ?>')"><i class="icon-edit"></i>删除</a></li>
        </ul>
        </td>
...
$("#shopName").autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: "<?php echo U('CollectShop/queryShops');?>",
			dataType: "json",
...
function addShops()
{
	...
	else
	{
		...
		{
			var url = "<?php echo U('addshop');?>" + "?lid=" + lineID + "&sid=" + shopID;
...
function delShops(lid, sid)
{
	...
	{
		var url = "<?php echo U('delshop');?>" + "?lid=" + lid + "&sid=" + sid;
		$.get(url, function(result){
			if(result == 1) {
				alert("删除成功!");
				location.reload(true);
