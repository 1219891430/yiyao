        ...
        <div class="modal-body modal_850">
                ...
                <tbody>
                    <tr>
                        <td width="80px">出库仓库：</td><td class="tl" colspan="3">
                        	<input id="depot_out_id" type="hidden" value="<?php echo ($depot_out_id); ?>" />
                        ...
                        <option value="1" <?php if($res["out_type"]==1){ echo "selected='selected'";} ?>>经销商出库</option>                 
                        <option value="2" <?php if($res["out_type"]==2){ echo "selected='selected'";} ?>>配送出库</option>         
                        <option value="3" <?php if($res["out_type"]==3){ echo "selected='selected'";} ?>>盘亏出库</option>       
                        <option value="4" <?php if($res["out_type"]==4){ echo "selected='selected'";} ?>>报损出库</option>
                       
                        </select>
                       
                        </td>
                      </tr>
                      <?php if($res['org_parent_id']){ ?>
                      <tr>
                      ...
                      <?php } ?>
                       <tr>
                        ...
                        <select name="send_staff_id" id="send_staff_id" class="w200 form-control" disabled="disabled">
                            ...
                            <?php if(is_array($psStafflist)): $i = 0; $__LIST__ = $psStafflist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dvo): $mod = ($i % 2 );++$i; if($res["send_staff_id"]==$dvo["admin_id"]){ ?>
                            	<option value="<?php echo ($dvo["admin_id"]); ?>" selected="selected"><?php echo ($dvo["true_name"]); ?></option>	
                            	<?php }else{ ?>
                                <option value="<?php echo ($dvo["admin_id"]); ?>"><?php echo ($dvo["true_name"]); ?></option>
                                <?php } endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                      </tr>
                    ...
                    <tr>
                        <td>出库备注：</td>
                        <td colspan="3"><input class="w300 form-control" value="<?php echo ($res["out_remark"]); ?>" id="out_remark" disabled="disabled"></td>
                    </tr>
                </tbody>
                <tfoot></tfoot>
            </table>
            <table class="table list_table" id="goods_table">
                ...
                <?php if(is_array($resGoods)): $i = 0; $__LIST__ = $resGoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gvo): $mod = ($i % 2 );++$i;?><tr class='tr_operate'>
                    ...
                    <td><input class='w50 tr goods_num' type='text' disabled="disabled" value='<?php echo (getGoodsNum($gvo["out_num"])); ?>'></td>
	...
	$("#brand").change(function(){
        ...
        function depotIn(){
            ...
            else
            {
                ...
                ajaxDataAUD("/index.php/Admin/DepotOut/outPassEx",data,true)
            }
        }
