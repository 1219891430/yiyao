...
<?php if(is_array($sub_memu)): $i = 0; $__LIST__ = $sub_memu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rightsubvo): $mod = ($i % 2 );++$i;?><dl>
    ...
    <dt></dt>
        ...
        <div class="row-fluid main-content">
            ...
            <div class="sel-data mb20">
                <div class="fl">
                    ...
                    <select id="staff_id" class="w100 form-control">
                        ...
                        <?php if(is_array($aStaff)): $i = 0; $__LIST__ = $aStaff;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$svo): $mod = ($i % 2 );++$i; if($svo['staff_id'] == $aUrlPara['staff_id']): ?><option selected="selected" value="<?php echo ($svo["staff_id"]); ?>"><?php echo ($svo["staff_name"]); ?></option>
                            <?php else: ?>
                                <option value="<?php echo ($svo["staff_id"]); ?>"><?php echo ($svo["staff_name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                   
                    </select>
                    <input type="text" class="w150 form-control" <?php if($aUrlPara['name'] != '@@'): ?>value="<?php echo ($aUrlPara["name"]); ?>"<?php endif; ?> id="cust_name" placeholder="请输入店铺名称">
                    <input type="text" readonly="readonly" <?php if($aUrlPara['start'] != 0): ?>value="<?php echo ($aUrlPara['start']); ?>"<?php endif; ?>  class="form-control w100 cursor-pointer" id="start_time"
                    placeholder="起始时间">
                    <input type="text" readonly="readonly" <?php if($aUrlPara['end'] != 0): ?>value="<?php echo ($aUrlPara['end']); ?>"<?php endif; ?> class="form-control w100 cursor-pointer" id="end_time"
                    placeholder="结束时间">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                       ...
                       <!--  <td><?php echo ($vo["status_name"]); ?></td> -->
                        ...
                        <td><ul class="operate-menu li-width50">
                                <li><a attr="<?php echo ($vo["change_id"]); ?>" class="collapsed collapse-menu icons-href detail" href="javascript:void(0)">
                                    <i class="icon-edit"></i>查看
                                </a></li>
                                <!--<li><a class="collapsed collapse-menu icons-href" href="<?php echo U('print_template',array('change_id' => $vo['change_id'],'del'=>$_GET['del']));?>" target="_blank"><i class="icon-edit"></i>打印</a>
                                </a></li>-->
            ...
            <!--表格查询结束-->
            <!--分页查询开始-->
            <?php echo W('Page/page',array("/index.php/Dealer/CarsaleChange/index",$pnum,$pagelist,$aUrlPara));?>
            <!--分页查询结束-->
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    //     ajaxDataPara("/index.php/Dealer/CarsaleChange/detail",data);
    ...
    $(".print").click(function(){
        ...
        location.href='/index.php/Dealer/CarsaleChange/print_template/change_id/'+change_id;
    ...
    $("#find").click(function(){
        ...
        if((start==0&&end!=0)||(start!=0&&end==0))
            ...
            location.href="/index.php/Dealer/CarsaleChange/index/staff_id/"+$("#staff_id").val()+"/name/"+cust_name+"/start_time/"+start+"/end_time/"+end+"/del/"+del;
    })
    $("#toGoodsCount").click(function(){
    	location.href="<?php echo U('ChangeGoodsCount/index');?>";
    });
</script>
</body>
</html>