    ...
    <dt></dt>
        ...
        <!--右侧查询开始-->
            <div class="sel-data mb20">
                <div class="fl">
                    <select class="form-control w150" name="staffId" id="staffId">
                        ...
                        <?php if(is_array($stafflist)): $i = 0; $__LIST__ = $stafflist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$staff): $mod = ($i % 2 );++$i; if($staff['staff_id'] == $staff_id): ?><option value="<?php echo ($staff["staff_id"]); ?>" selected="selected" ><?php echo ($staff["staff_name"]); ?></option>
                                <?php else: ?><option value="<?php echo ($staff["staff_id"]); ?>"><?php echo ($staff["staff_name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                    <input type="text" class="form-control w150" value="<?php echo ($shop); ?>" id="shop" placeholder="商铺名称"/>
                    <input type="text" readonly="readonly" class="form-control w150" value="<?php echo ($start); ?>" id="start_time"
                           placeholder="起始时间">
                    <input type="text" readonly="readonly" class="form-control w150" value="<?php echo ($end); ?>" id="end_time"
                           placeholder="结束时间">
            ...
            <table class="table list_table" id="role_table">
                ...
                <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr id="tr_<?php echo ($list["log_id"]); ?>">
                        <td style="width:15%"><?php echo ($list["staff_name"]); ?></td>
                        <td style="width:15%"><?php echo ($list["cust_name"]); ?></td>
                        <td style="width:40%">
                            <?php if($list["record"]==1): ?>
                            <a href="/Public/Uploads/<?php echo ($list["file_path"]); ?>" target="_blank">语音日志</a>
                            <audio src="/Public/Uploads/<?php echo ($list["file_path"]); ?>">您的浏览器不支持 audio 标签。</audio>
                            <?php else:?>
                            <?php echo ($list["log_content"]); ?>
                            <?php endif;?>
                        </td>
                        <td style="width:15%"><?php echo ($list["log_time"]); ?></td>
                        <td style="width:15%"><ul class="operate-menu li-width100">
                            <li><a attr="<?php echo ($list["role_id"]); ?>" class="collapsed collapse-menu icons-href delete" id="<?php echo ($list["log_id"]); ?>" href="javascript:void(0)">
            ...
            <?php echo W('Page/page',array("/index.php/Dealer/Shoplog/index",$pnum,$pagelist,array('staff_id'=>$staff_id,'start'=>$start,'end'=>$end,'shop'=>$shop)));?>
            <!--分页查询结束-->
        </div>
    </div>
</div>
    ...
    $(function () {
        ...
        $(".delete").click(function(){
            if(confirm("确定要删除该记录吗？"))
            {
                ...
                $.post("/index.php/Dealer/Shoplog/delLog",{log_id:id},function(result){
