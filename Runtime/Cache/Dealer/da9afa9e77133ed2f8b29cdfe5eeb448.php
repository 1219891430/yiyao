    ...
    <div class="row-fluid main-content">
        <!--右侧查询开始-->
            <div class="sel-data mb20">
                <div class="fl">
                    ...
                    <select id="status" class="w100 form-control">
                        ...
                        <?php if(is_array($aStatus)): $k = 0; $__LIST__ = $aStatus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($k % 2 );++$k; if($k == $urlPara['status']): ?><option selected="selected" value="<?php echo ($k); ?>"><?php echo ($ivo); ?></option>
                                <?php else: ?>
                    ...
                    <select id="way" class="w100 form-control">
                        ...
                        <?php if(is_array($aWay)): $k = 0; $__LIST__ = $aWay;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($k % 2 );++$k; if($k == $urlPara['way']): ?><option selected="selected" value="<?php echo ($k); ?>"><?php echo ($ivo); ?></option>
                            <?php else: ?>
                            <option value="<?php echo ($k); ?>"><?php echo ($ivo); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                    <select id="tType" class="w100 form-control">
                        <option value="0">订单类型</option>
                        <?php if(is_array($tType)): $k = 0; $__LIST__ = $tType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($k % 2 );++$k; if($k == $urlPara['way']): ?><option selected="selected" value="<?php echo ($k); ?>"><?php echo ($ivo); ?></option>
                            <?php else: ?>
                            <option value="<?php echo ($k); ?>"><?php echo ($ivo); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                    <select id="Show" class="w100 form-control">
                        ...
                        <option <?php if(1 == $_GET['Show']): ?>selected="selected"<?php endif; ?> value="1">有备注订单</option>
                     
                    </select>
                    <div class="w250" style="display: inline-block;position: relative;margin-top:0px;">

                            <input name="cust_names"  <?php if($urlPara['name'] != '@@'): ?>value="<?php echo ($urlPara['name']); ?>"<?php endif; ?> id="cust_names" type="text" placeholder="请输入店铺名称" class="form-control w250">
            ...
            <table class="table list_table" id="role_table">
                ...
                <tbody>
                <?php if(is_array($aOrder)): $i = 0; $__LIST__ = $aOrder;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($vo["order_code"]); ?></td>
                        <td><?php echo ($vo["cust_name"]); ?></td>
                        <td><?php echo ($vo["cust_contact"]); ?></td>
                        <td><?php echo ($vo["staff_name"]); ?></td>
                        <td><?php echo ($vo["order_real_money"]); ?></td>
                       ...
                       <!--  <td><?php echo ($vo["status_name"]); ?></td> -->
                        ...
                        <td><ul class="operate-menu li-width50">
                                <li><a attr="<?php echo ($vo["attr"]); ?>" data-name="<?php echo ($vo["attr_id"]); ?>" class="collapsed collapse-menu icons-href detail" href="javascript:void(0)">
                                        <i class="icon-edit"></i>查看
                                    </a>
                                </li>
                                <li>
                                    <?php if(!empty($vo["is_show"])): ?><img style="height:17px;width:17px;" src="/Public/images/beizhu.png" alt=""><?php endif; ?>
                                </li>
                        </ul></td>
            ...
            <!--表格查询结束-->
            <!--分页查询开始-->
            <!-- <?php echo W('Page/page',array("/index.php/Dealer/OrderReport/index",$pnum,$pagelist,$urlPara));?>-->
            <!--分页查询结束-->
        </div>
